...
template ColorSort(NTUBES, CAP, NMOVES, NCOLORS, EMPTY_TUBES, TUBE_BITS, IDX_BITS, COLOR_BITS) {
    signal input initial[NTUBES * CAP]; // private
    signal input sender[32];            // private
    signal input puzzlePacked;          // public
    signal input senderHi;              // public
    signal input senderLo;              // public
    ...
    // Sender bytes must be valid uint8 values.
    component senderInRange[32];
    for (var i = 0; i < 32; i++) {
        senderInRange[i] = LessThan(9);
        senderInRange[i].in[0] <== sender[i];
        senderInRange[i].in[1] <== 256;
        senderInRange[i].out === 1;
    }

    // Pack first 10 tubes (40 slots) into 20 bytes, then into one big-endian field.
    signal puzzleByte[20];
    for (var t = 0; t < 10; t++) {
        puzzleByte[t * 2] <== initial[t * CAP] * 16 + initial[t * CAP + 1];
        puzzleByte[t * 2 + 1] <== initial[t * CAP + 2] * 16 + initial[t * CAP + 3];
    }

    signal puzzlePackAcc[21];
    puzzlePackAcc[0] <== 0;
    for (var i = 0; i < 20; i++) {
        puzzlePackAcc[i + 1] <== puzzlePackAcc[i] * 256 + puzzleByte[i];
    }
    puzzlePackAcc[20] === puzzlePacked;

    // Pack sender bytes into two 16-byte big-endian field elements.
    signal senderHiAcc[17];
    senderHiAcc[0] <== 0;
    for (var i = 0; i < 16; i++) {
        senderHiAcc[i + 1] <== senderHiAcc[i] * 256 + sender[i];
    }
    senderHiAcc[16] === senderHi;

    signal senderLoAcc[17];
    senderLoAcc[0] <== 0;
    for (var i = 0; i < 16; i++) {
        senderLoAcc[i + 1] <== senderLoAcc[i] * 256 + sender[16 + i];
    }
    senderLoAcc[16] === senderLo;
...
component main { public [puzzlePacked, senderHi, senderLo] } = ColorSort(12, 4, 120, 10, 2, 4, 3, 5);
